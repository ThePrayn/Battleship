...
const InfoBoard = ( {
  ...
  return (
    <div className='infoBoard'>
      <p>{winnerInfo ? winnerInfo : 'Размести свои 5 кораблей и начинай стрелять. Правая кнопка мыши поворачивает размещаемый корабль'}</p>
      <div className='btnHolder'>
        <button className='button restartBtn' onClick={() => restartBtnHandler()}>{winnerInfo ? 'Сыграть снова' : 'Рестарт'}</button>
      </div>
    </div>
  );
};
